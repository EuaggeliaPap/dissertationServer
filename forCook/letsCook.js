        ...
        if (!err) {
            if (rows.length === 4) {
                ...
                for (var i = 0; i < subids.length; i++) {
                    ...
                    // ------------------------------------H sub ", i + 6, " exei tis ", subsrecs[i], "suntages"
                }
                var ssid;
                for (var i = 0; i < catids.length; i++) {
...
// ------------------------------------------- sendEmail ---------------------------------------------------------------
app.post("/sendInfoEmail", function (req, res) {
    var userEmail = req.body.fromEmail;
    var emailText = req.body.emailText;
    var firstName = req.body.firstName;
    var lastName = req.body.lastName;
    sendEmail.sendEmail('baggeliw_pap@yahoo.gr', '0803081749211191bg', 'euaggeliap@yahoo.gr', userEmail, emailText);
    sendEmail.sendEmail('baggeliw_pap@yahoo.gr', '0803081749211191bg', userEmail, 'Let \'s cook App ', 'κ. ' +
        firstName + ' ' + lastName + '<br/> Το email ' +
        'σας στάλθηκε με επιτυχία! Ευχαριστούμε, θα λάβετε σύντομα απάντηση μας! <br/> Με εκτίμηση, <br/> Ευαγγελία Παπαγεωργίου');
...
app.post("/updateUser", function (req, res) {
    ...
    var sql = "call sp_updateUser(?, ?, ?, ?, ?)";
    connection.query(sql,[data.name, data.surname, data.email, data.password, data.oldemail], function (err, rows) {
        if (!err) {
